...
async function main() {
  ...
  // ====================================================================
  {
    ...
    // at least one table at every stakes tier. Look up by NAME in the
    // actual rooms.tables Map (NOT listTables()) because listTables()
    // returns the public view (id, name, seatsTaken, maxSeats, …) which
    // has no `.seats`, no `.default`, and no settable `.phase` — those
    // fields live on the internal table object only. Using listTables()
    // here was a prior bug that crashed the test runner with
    // "Cannot read properties of undefined (reading 'forEach')".
    rooms.ensureDefaultTables();
    const beginner = Array.from(rooms.tables.values()).find(
      (t) => t.name === 'Beginners Table'
    );
